            ...
            with open(result_filepath, 'w') as file, redirect_stdout(file):
                ...
                for i in output_active_list:
                    ...
                    if os.path.exists(knowledge_filepath):
                        ...
                        print("\nIndividual run for target word: %s" % vectorizer_X.get_feature_names_out()[i])
                        training_time, target_word_clauses = knowledge.generate(feature_names, X_train, current_folder_path, i)
                            ...
                            if weight > 0:
                                ...
                                for literal in related_literals:
                                    ...
                                    if os.path.exists(knowledge_filepath):
                                        ...
                                        print("\nIndividual run for target word: %s" % vectorizer_X.get_feature_names_out()[literal])
                                        training_time, inner_target_word_clauses = knowledge.generate(feature_names, X_train, current_folder_path, literal)
                                        total_training_time = total_training_time + training_time
